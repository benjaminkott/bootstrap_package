...
class ext_update {
    ...
    public function main() {
        $rows = $this->getDatabaseConnection()->exec_SELECTgetRows(
                ...
                'pages',
                '
                       backend_layout LIKE \'' . self::OLD_BACKEND_LAYOUT_PREFIX . '%\'
                    OR backend_layout_next_level LIKE \'' . self::OLD_BACKEND_LAYOUT_PREFIX . '%\'
                ');

        foreach ($rows as $row) {
			if (strpos($row['backend_layout'], self::OLD_BACKEND_LAYOUT_PREFIX) !== FALSE) {
				$fieldsToUpdate['backend_layout'] = substr_replace($row['backend_layout'],
						self::NEW_BACKEND_LAYOUT_PREFIX, 0, strlen(self::OLD_BACKEND_LAYOUT_PREFIX));
			}
			if (strpos($row['backend_layout_next_level'], self::OLD_BACKEND_LAYOUT_PREFIX) !== FALSE) {
				$fieldsToUpdate['backend_layout_next_level'] = substr_replace($row['backend_layout_next_level'],
						self::NEW_BACKEND_LAYOUT_PREFIX, 0, strlen(self::OLD_BACKEND_LAYOUT_PREFIX));
			}
