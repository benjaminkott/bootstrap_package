 ...
 * When step > 1 return slices of base array of step size for each iteration
 * = Examples =
 *
 ...
 * @api
 */
class ForStepViewHelper extends \TYPO3\CMS\Fluid\Core\ViewHelper\AbstractViewHelper implements \TYPO3\CMS\Fluid\Core\ViewHelper\Facets\CompilableInterface
{
    /**
     ...
     * @param integer $step step between elements, when more than 1 return a sliced array
     * @return string Rendered string
     * @api
     */
    public function render($each, $as, $key = '', $reverse = false, $iteration = null, $step = null)
    {
    ...
    $output = '';
    if ($arguments['step'] == null or $arguments['step'] < 2){
      ...
      $last = count($arguments['each'])-1;
      $multipleElements = array();
      foreach ($arguments['each'] as $keyValue => $singleElement) {
        $multipleElements[] = $singleElement;
        if (($keyValue > 0 and $keyValue % $arguments['step'] === 0) or $keyValue === $last){
          $templateVariableContainer->add($arguments['as'], $multipleElements);
          if ($arguments['key'] !== '') {
            $templateVariableContainer->add($arguments['key'], $keyValue);
          }
          if ($arguments['iteration'] !== null) {
